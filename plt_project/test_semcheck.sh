...
testbenches_dir="testing_semcheck/test/"
expected_outputs_dir="testing_semcheck/expected_outputs/"
outputs_dir="testing_semcheck/output/"
...
for test_file in ${testbenches_dir}*.tb; do
    ...
    if [[ $test_base == *"test"* ]]; then
        ...
        ./bin/semant_eval < "${testbenches_dir}${test_base}" > "${outputs_dir}${test_base%.*}.out" 2> temp_error.out
