    ...
    | SBinop (e1, op, e2) ->
      ...
      match t, op with
        ...
        | Int, Mul   -> L.build_mul e1' e2' "multmp" builder *)
         (* Signed division *)
        (* | Int, Div   -> L.build_sdiv e1' e2' "divtmp" builder *)
         (* Signed remainder *)
        (* | Int, Mod   -> L.build_srem e1' e2' "modtmp" builder *)
...
(* LLVM insists each basic block end with exactly one "terminator"
    ...
    let rec build_sstmt builder sstmt env = begin match sstmt with
        (* SBlock sl -> build_sstmt_list builder sl env *)
      | SExpr e -> ignore(build_expr builder e env);
  end
  ...
  (* let build_stmt builder = function
    ...
    | A.Block sl -> build_sstmt_list builder sl
    | A.Expr e -> ignore(build_expr builder e); *)
